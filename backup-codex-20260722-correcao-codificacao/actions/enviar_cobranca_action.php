...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    set_flash_message('error', "MÃ©todo invÃ¡lido.");
...
if (!$id_conta) {
    set_flash_message('error', "Conta nÃ£o identificada.");
    header('Location: ../pages/contas_receber.php');
    exit;
}
...
if ($conn === null) {
    set_flash_message('error', "Erro de conexÃ£o.");
    header('Location: ../pages/contas_receber.php');
    exit;
}
...
try {
    ...
    if (!$conta) throw new Exception("Conta nÃ£o encontrada.");
    ...
    // Corpo do E-mail
    $mail->isHTML(true);
    $mail->Subject = "CobranÃ§a / Fatura - $nome_empresa";
    ...
    <div style='font-family: Arial, sans-serif; color: #333; max-width: 600px; border: 1px solid #ddd;'>
        <div style='background-color: #ffc107; color: #333; padding: 20px; text-align: center;'>
            <h2 style='margin:0;'>Aviso de CobranÃ§a</h2>
        </div>
        <div style='padding: 30px;'>
            <p>OlÃ¡, <strong>{$conta['nome_cliente']}</strong>.</p>
            <p>Seguem os dados da fatura pendente:</p>
            
            <div style='border-left: 4px solid #ffc107; padding-left: 15px; margin: 20px 0;'>
                <p><strong>DescriÃ§Ã£o:</strong> {$conta['descricao']}</p>
            ...
            <p style='font-size:0.9em; color:#666; margin-top:30px;'>
                Se jÃ¡ efetuou o pagamento, por favor desconsidere este e-mail.
                Em anexo, caso necessÃ¡rio, segue o documento relacionado.
            </p>
        </div>
    </div>";
    ...
    $mail->send();
    // SUCESSO
    set_flash_message('success', "CobranÃ§a enviada com sucesso!");
