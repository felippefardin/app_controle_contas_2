<?php
// ConfiguraÃ§Ã£o de erro para retornar JSON limpo
...
try {
    ...
    if (!isset($_SESSION['usuario_logado'])) {
        throw new Exception('SessÃ£o expirada. FaÃ§a login novamente.');
    }
    
    // --- CÃ“DIGO NOVO: BLOQUEIO DO PLANO BÃSICO ---
    // Isso garante que NINGUÃ‰M no plano bÃ¡sico (nem admin) consiga finalizar uma venda.
    $plano = $_SESSION['plano'] ?? 'basico';
    if ($plano === 'basico') {
        throw new Exception('Seu plano atual (BÃ¡sico) nÃ£o permite registrar vendas. FaÃ§a um upgrade para continuar.');
    }
    // ------------------------------------------------

    $conn = getTenantConnection();
    if ($conn === null) {
        throw new Exception('Falha de conexÃ£o com o banco de dados.');
    }
    ...
    if (empty($itens_venda) || !is_array($itens_venda)) {
        throw new Exception('Nenhum item vÃ¡lido na venda.');
    }
    if (empty($cliente_id)) {
    ...
    foreach ($itens_venda as $item) {
        ...
        if (!$produto) throw new Exception("Produto ID $id_produto nÃ£o encontrado.");
    ...
    // 7. LANÃ‡AMENTO FINANCEIRO (CONTAS A RECEBER)
    ...
    // DescriÃ§Ã£o inclui o ID da venda para rastreamento
    ...
    if (!$stmt_fin->execute()) {
        throw new Exception("Erro ao lanÃ§ar no financeiro: " . $stmt_fin->error);
    }
    $stmt_fin->close();

    // Notifica toda a equipe uma Ãºnica vez quando a meta mensal for alcanÃ§ada.
    $chaveMeta = 'meta_vendas_' . date('Y_n');
    $stmtMeta = $conn->prepare('SELECT valor FROM configuracoes_tenant WHERE chave=? LIMIT 1');
    $stmtMeta->bind_param('s', $chaveMeta); $stmtMeta->execute();
    $metaRow = $stmtMeta->get_result()->fetch_assoc();
    $valorMeta = (float)($metaRow['valor'] ?? 0);
    if ($valorMeta > 0) {
        $totalMes = (float)($conn->query("SELECT COALESCE(SUM(valor_total),0) total FROM vendas WHERE YEAR(data_venda)=YEAR(CURDATE()) AND MONTH(data_venda)=MONTH(CURDATE())")->fetch_assoc()['total'] ?? 0);
        if ($totalMes >= $valorMeta) {
            $tituloMeta = 'ParabÃ©ns, equipe! Meta mensal alcanÃ§ada! ðŸŽ‰';
            $mensagemMeta = 'A equipe alcanÃ§ou a meta de R$ ' . number_format($valorMeta,2,',','.') . '. Total realizado: R$ ' . number_format($totalMes,2,',','.') . '. Obrigado pelo empenho de todos!';
