...
// 2. VERIFICA SE O MÃ‰TODO Ã‰ POST E O TOKEN
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // Verifica CSRF (SeguranÃ§a)
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        set_flash_message('danger', 'Token de seguranÃ§a invÃ¡lido. Tente novamente.');
    ...
    if ($conn === null) {
        set_flash_message('danger', 'Falha na conexÃ£o com o banco de dados.');
        header('Location: ../pages/contas_receber.php');
        exit;
    }
    ...
    if (!$usuario_id) {
        set_flash_message('danger', 'SessÃ£o expirada. FaÃ§a login novamente.');
    ...
    // Usa funÃ§Ãµes do utils.php para garantir que R$ 1.000,00 vire 1000.00 e datas fiquem certas
    ...
    if ($valor <= 0 || empty($data_vencimento) || empty($id_categoria) || empty($descricao)) {
        set_flash_message('danger', 'Preencha a descriÃ§Ã£o, categoria, data e um valor vÃ¡lido.');
        header('Location: ../pages/contas_receber.php');
        exit;
    }
    ...
    if ($stmt) {
        ...
        set_flash_message('danger', 'Erro tÃ©cnico ao preparar consulta.');
    }
