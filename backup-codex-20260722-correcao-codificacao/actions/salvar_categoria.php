...
if (!isset($_SESSION['usuario_logado']) || $_SESSION['usuario_logado'] !== true) {
    // Se for AJAX, retorna erro JSON, senÃ£o redireciona
    if (isset($_POST['ajax'])) {
        echo json_encode(['status' => 'error', 'message' => 'UsuÃ¡rio nÃ£o logado']);
        exit;
    }
...
// 2. VERIFICA MÃ‰TODO POST E CSRF
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // Verifica se Ã© chamada AJAX
    ...
    if (empty($id)) { 
        ...
        // Atualizar (Garante propriedade do usuÃ¡rio)
    ...
    if ($stmt) {
        ...
        if ($isAjax) {
            echo json_encode(['status' => 'error', 'message' => 'Erro na preparaÃ§Ã£o da query']);
            exit;
        }
