...
// 1. VERIFICA O LOGIN E PEGA A CONEXÃƒO CORRETA
// â—ï¸ CORREÃ‡ÃƒO: Verifica se o login Ã© 'true', e nÃ£o apenas se 'isset'
...
// â—ï¸â—ï¸ INÃCIO DA CORREÃ‡ÃƒO â—ï¸â—ï¸
// Pega o ID do usuÃ¡rio logado e o ID do cliente/fornecedor da URL
$id_usuario = get_data_owner_id(); // Linha 18 corrigida
// â—ï¸â—ï¸ FIM DA CORREÃ‡ÃƒO â—ï¸â—ï¸
$id_pessoa_fornecedor = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if ($id_pessoa_fornecedor === 0) {
    echo "<div class='container'><h1>ID invÃ¡lido.</h1></div>";
    exit;
}

include('../includes/header.php');

// 2. BUSCA OS DADOS COM SEGURANÃ‡A, FILTRANDO PELO USUÃRIO
// (Agora $id_usuario estÃ¡ correto)
...
$nome_pessoa = $pessoa ? $pessoa['nome'] : 'NÃ£o encontrado';

// HistÃ³rico de contas a pagar (usando a coluna correta `usuario_id`)
...
// HistÃ³rico de contas a receber (usando a coluna correta `usuario_id`)
...
// HistÃ³rico de estoque (usando a coluna correta `id_usuario`)
...
<head>
    ...
    <title>HistÃ³rico de <?= htmlspecialchars($nome_pessoa) ?></title>
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1><i class="fa-solid fa-history"></i> HistÃ³rico de: <?= htmlspecialchars($nome_pessoa) ?></h1>
