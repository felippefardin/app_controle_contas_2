...
// ----------------------------
// 4. INCLUDES DE CABEÃ‡ALHO
// ----------------------------
include('../includes/header.php');

// âœ… EXIBE O FLASH MESSAGE DO PHP (Se houver msg na sessÃ£o)
...
.container {
    ...
    /* Full Desktop: ocupa quase toda a largura, mas mantÃ©m margem */
    max-width: 98%; 
    width: 100%;
}
...
.table {
    ...
    white-space: nowrap; /* Garante que nÃ£o quebre em mobile */
}
...
/* =========================================
   ALERTAS E NOTIFICAÃ‡Ã•ES (CORREÃ‡ÃƒO)
========================================= */
.alert {
    border-radius: 6px;
    font-weight: 500;
...
/* CSS para o Flash Message JS (CorreÃ§Ã£o) */
.alert-overlay {
    position: fixed;
    top: 20px;
    right: 20px;
...
@media (max-width: 768px) {
    ...
    /* BotÃµes de AÃ§Ã£o (Recibo e NFe) */
    #btn-recibo, #btn-nfe {
        width: 100%;
        display: block;
...
<div class="container">
    ...
    <div class="meta-widget">
        <div class="meta-header">
            <span class="meta-header-titulo"><i class="fas fa-bullseye"></i> Meta de Vendas (MÃªs)</span>
            <?php if ($perfil === 'admin' || $perfil === 'proprietario'): ?>
                <span id="btn-editar-meta" class="meta-header-editar" data-toggle="modal" data-target="#modalMetaVendas" title="Definir Meta do MÃªs">
                ...
                <div class="form-row align-items-end">
                    ...
                    <div class="form-group col-md-4">
                        <button type="button" id="add-produto" class="btn btn-success btn-block">
                            <i class="fas fa-plus"></i> Adicionar Ã  Venda
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content" style="background-color: #222; color: #eee;">
            <div class="modal-header" style="border-bottom: 1px solid #444;">
                <h5 class="modal-title" id="modalMetaLabel">Definir Meta de Vendas do MÃªs</h5>
...
$(document).ready(function() {
    ...
    // =======================================================
    // âœ… FUNÃ‡ÃƒO DE ALERTA FLUENTE VIA JAVASCRIPT (CORRIGIDA)
    // =======================================================
    function showAlert(message, type) {
        ...
        // Fecha automaticamente apÃ³s 4 segundos
        setTimeout(() => {
    ...
    function carregarMetaVendas() {
        ...
        .then(data => {
            if (data.success) {
                ...
                console.warn('NÃ£o foi possÃ­vel carregar a meta de vendas: ' + data.message);
            }
        })
        .catch(err => {
            console.error('Erro ao buscar meta:', err);
            // Opcional: showAlert('Erro de comunicaÃ§Ã£o ao buscar meta de vendas.', 'danger');
        });
    }
    ...
    if (userPerfil === 'admin' || userPerfil === 'proprietario') {
        ...
        $('#btn-salvar-meta').on('click', function() {
            ...
            .catch(err => {
                ...
                showAlert('Erro de comunicaÃ§Ã£o ao salvar a meta.', 'danger');
            });
        });
    }
    ...
    $('#form-novo-cliente').on('submit', function(e) {
        ...
        // Flag para indicar que Ã© via AJAX
        ...
        $.ajax({
            ...
            success: function(response) {
                // Como o backend redireciona, o 'success' Ã© chamado.
                ...
                // Limpa o select para o usuÃ¡rio buscar o novo cliente
                $('#cliente_id').val(null).trigger('change');
                
                // Se quiser prÃ©-selecionar o cliente recÃ©m criado:
                const nome = formData.get('nome');
                // Mas nÃ£o temos o ID, entÃ£o apenas forÃ§amos o usuÃ¡rio a buscar.
            },
            error: function() {
    ...
    $('#add-produto').on('click', function() {
        ...
        if ($(`#venda-items tr[data-id="${produtoId}"]`).length > 0)
            return showAlert('Este produto jÃ¡ foi adicionado.', 'warning');

        if (estoque <= 0)
            return showAlert('Este produto estÃ¡ sem estoque.', 'danger');
    ...
    $('#venda-items').on('input', '.quantidade', function() {
        ...
        if (qtd > estoque) {
            showAlert(`âš ï¸ Estoque mÃ¡ximo disponÃ­vel: ${estoque}`, 'warning');
            $(this).val(estoque);
            qtd = estoque;
        }
    ...
    $('#form-venda').on('submit', function(e) {
        ...
        if ($('#venda-items tr').length === 0)
            return showAlert('Adicione pelo menos um produto Ã  venda.', 'danger');
    ...
    function emitirNFe(vendaId) {
        ...
        showAlert('â³ Gerando NFC-e de SimulaÃ§Ã£o...', 'info');
        ...
        .then(data => {
            if (data.success) {
                showAlert('âœ… SIMULAÃ‡ÃƒO: NFC-e gerada com sucesso! Chave FictÃ­cia: ' + data.chave, 'success');
                setTimeout(() => {
                    window.open('../actions/gerar_danfe.php?chave=' + data.chave, '_blank');
                }, 1500);
            } else
                showAlert('âŒ Erro na simulaÃ§Ã£o da NFC-e: ' + data.message, 'danger');
        })
        .catch(err => {
            console.error(err);
            showAlert('Erro de comunicaÃ§Ã£o com o servidor na emissÃ£o da NF-e.', 'danger');
        });
    }
