...
if (!$conn) {
    die("Falha crÃ­tica: NÃ£o foi possÃ­vel conectar ao banco de dados do cliente.");
}
...
if (isset($_GET['id'])) {
    ...
    if ($stmt = $conn->prepare("SELECT * FROM produtos WHERE id = ? AND id_usuario = ?")) {
        ...
        if ($stmt->execute()) {
            ...
            if ($result->num_rows > 0) {
                ...
                set_flash_message('danger', "Produto nÃ£o encontrado ou vocÃª nÃ£o tem permissÃ£o.");
    ...
    set_flash_message('danger', "ID nÃ£o informado.");
    header("Location: controle_estoque.php");
    exit;
}
    ...
    <div class="container">
        ...
        <?php elseif ($produto && is_array($produto)): ?>
            ...
            <form action="../actions/editar_produto_action.php" method="POST">
                ...
                <h5>InformaÃ§Ãµes Fiscais (Opcional)</h5>
                ...
                <div class="mt-3">
                    <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Salvar AlteraÃ§Ãµes</button>
